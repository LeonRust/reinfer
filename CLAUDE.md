...
8. **模型获取（013 定案）**：模型默认从 **ModelScope** 下载（铁律主体）；`REINFER_MODEL_SOURCE=auto`（缺省）语义 = ModelScope 优先、缺（404/文件缺失）→ HuggingFace 回退（2026-08-27 用户明确放开）。纯 Rust 实现（`reinfer model list/get` + 运行时 `ModelResolver`）——**不得引入 pip/Python**；命令示例：`reinfer model list`（本地）/ `reinfer download <repo> [-q qtag|--include glob|file...|全量] [--revision ref] [--local-dir dir]` —— CLI 契约以 docs/design/cli-contract-2026-08-27.md 为准（env 表见 `specs/013-model-fetch/plan.md` D6.1）。**代码零硬编码模型标识**：repo/文件名/量化段一律来自 CLI/env/`ModelSpec`（测试用虚构 repo 与本地 stub）；真实仓库名仅可出现在文档示例。网络出口用标准 `HTTP(S)_PROXY`/`NO_PROXY`（不硬编码代理 IP）；`REINFER_MODEL_AUTODOWNLOAD=off` 保证引擎绝不联网。
